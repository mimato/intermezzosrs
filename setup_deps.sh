...
if [ ! -d "gcc-5.3.0" ]; then
  ...
  mkdir -p build-gcc
  cd build-gcc
  ../gcc-5.3.0/configure --target=$TARGET --prefix="$PREFIX" --disable-nls --enable-languages=c,c++ --without-headers --with-gmp=/usr/local/Cellar/gmp/6.1.1 --with-mpfr=/usr/local/Cellar/mpfr/3.1.5 --with-mpc=/usr/local/Cellar/libmpc/1.0.3
  make all-gcc
